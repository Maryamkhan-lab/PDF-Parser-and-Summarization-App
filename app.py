...
# Set OpenAI and Pinecone API keys (hardcoded here)
OPENAI_API_KEY = "YOUR API KEY"
PINECONE_API_KEY = "YOUR PINE CONE API"
...
vectorstore = LangchainPinecone(index=index, embedding=embeddings, text_key="page_content")
...
if st.session_state.get('pdf_processed', False):
    ...
    if st.session_state['pdf_elements']['texts'] and not st.session_state['pdf_elements']['text_summaries']:
        ...
        with st.spinner('Summarizing tables...'):
            for table in st.session_state['pdf_elements']['tables']:
                summary = (prompt | model | StrOutputParser()).invoke({"element": table})
...
def prompt_func(data):
    ...
    if data["context"]["images"]:
        return [
            HumanMessage(
                content=[
                    ...
                    {format_texts}
                    """},
                    {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{data['context']['images'][0]}"}},
                ]
            )
        ]
    else:
        return [
            HumanMessage(
                content=[
                    {"type": "text", "text": f"""Answer the question based only on the following context, which can include text and tables:
                    Question: {data["question"]}
...
chain = (
    ...
    | RunnableLambda(lambda x: {"context": {"texts": x["context"]["texts"], "images": x["context"]["images"]}, "question": x["question"]})
...
if 'text_summaries' in st.session_state['pdf_elements'] and st.session_state['pdf_elements']['text_summaries']:
    st.download_button(label="Download Summaries CSV", data=download_summary_csv(), file_name="summaries.csv", mime="text/csv")
